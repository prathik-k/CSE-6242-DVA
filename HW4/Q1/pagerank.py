...
"""
def author():                                                                                             
		return 'pkaundinya3' # replace gburdell3 with your Georgia Tech username.                                                                                             
																							  
def gtid():                                                                                               
	return 903133534 # replace with your GT ID number      
...
class PageRank:
	...
	def calculate_node_degree(self):
		for source,target in self.read_edge_file(self.edge_file):
			...
			elif source in self.node_degree:
				self.node_degree[source] += 1				
			self.max_node_id = max(source,target,self.max_node_id)
	...
	def run_pagerank(self, node_weights, damping_factor=0.85, iterations=10):
		...
		for it in range(iterations):			
			new_scores = np.zeros(self.max_node_id + 1).tolist()
			for source, target in self.read_edge_file(self.edge_file):
				new_scores[target] += damping_factor*scores[source]/self.node_degree[source]

			min_pr = (1-damping_factor)*node_weights			
			scores = np.add(new_scores,min_pr)
